...
def main(myArgs):
    ...
    parser.add_option("-p", help="storage pool: supports {two, one, or perf}")
    ...
    if options.p is None:
        ...
        thePool = "pool_two"
    elif options.p == "three":
        thePool = "pool_three"
    else:
        subprocess.call('clear', shell=True)
        print sbcolors.sbRed + "Unknown pool name. Expecting: one, two, or three" + sbcolors.sbEnd
        quit()
...
def helper():
    ...
    print "-p <pool-name{two, one, perf}> -d <disk-for-pool>"
    print ""
    print "Example:"
